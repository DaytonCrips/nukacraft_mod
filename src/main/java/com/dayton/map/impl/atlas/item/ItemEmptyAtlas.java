package com.dayton.map.impl.atlas.item;
...
public class ItemEmptyAtlas extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        AtlasData atlasData = AntiqueAtlasMod.tileData.getData(atlasID, level);
        atlasData.getWorldData(player.getCommandSenderWorld().dimension()).setBrowsingPositionTo(player);
        atlasData.setDirty();

        MarkersData markersData = AntiqueAtlasMod.markersData.getMarkersData(atlasID, level);
